...
def create_models(file_path, models_name):
    ...
    # model_name_deepcase = model_name.get("DEEPCASE", None)
...
def create_model_IF(file_path, model_name):
    try:
        ...
        df = pd.read_csv(file_path, sep=";", names= ["event","machine","timestamp","label"])
        ...
        print(type(X_train))
        model_if = IsolationForest(contamination=0.1)  # Contamination rappresenta la percentuale di anomalie previste nel dataset
...
def create_model_DEEPCASE(file_path, model_name):
    try:
        ...
        interpreter.save(f"models/deepcase/f{model_name}")
        return interpreter
...
if __name__ == "__main__":
    ...
    models_name = {"IF":'isolation_forest_model.joblib', "DEEPCASE":'Interpreter.save'}
    create_models(file_path="logs/logFileBaseline.csv", models_name = models_name)


def evalute_model_IF(model, test_df):
    ...
    etichette_effettive = [1 if etichetta == "Normal" else -1 for etichetta in test_df['Status']]
    matrice_confusione = confusion_matrix(etichette_effettive, anomalie_test_if)
    plot_confusion_matrix(matrice_confusione, class_names=["Anomaly","Normal"])
    ...
    print(classification_report(etichette_effettive, anomalie_test_if))
...
def evalute_model_DEEPCASE(interpreter, file_path):
    ...
    predictions = interpreter.predict(
        X          = context_test_n,               # Context to predict
        y          = events_test_n.reshape(-1, 1), # Events to predict, note that these should be of shape=(n_events, 1)
        iterations = 100,                        # Number of iterations to use for attention query, in paper this was 100
        batch_size = 1024,                       # Batch size to use for attention query, used to limit CUDA memory usage
        verbose    = True,                       # If True, prints progress
    )
...
def normalize_tensor(tensor_data):
  ...
  new_min = 0
  new_max = 99
  normalized_tensor_specific_range = (tensor_data - tensor_min) / (tensor_max - tensor_min) * (new_max - new_min) + new_min
...
def plot_confusion_matrix(conf_matrix, class_names):
    ...
    plt.ylabel('Actual')
    plt.save("logs/confusion_matrix_IF")
